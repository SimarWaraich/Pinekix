...
public class ViewThisProfileForSocietyParty : MonoBehaviour
{
	...
	void ShowThis ()
	{
		...
		var thisPlayer = this.gameObject.transform.parent.parent.GetComponent<SocietyNetworkPlayer> ();
		int playerId = thisPlayer.PlayerDataForSocietyParty.player_id; 
		string username = thisPlayer.PlayerDataForSocietyParty.Username; 
		...
		FriendProfileManager.Instance.BackToSocietyParty.SetActive (true);
		...
		for (int i = 0; i < FriendProfileManager.Instance.BlockedByList.Count; i++) {
			if (FriendProfileManager.Instance.BlockedByList [i].PlayerId == playerId) {
				...
				this.transform.parent.parent.transform.GetComponent<SocietyRemovePlayer> ().ObjState = false;
